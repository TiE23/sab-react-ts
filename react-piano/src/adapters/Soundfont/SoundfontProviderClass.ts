...
type ProviderProps = {
  instrument: InstrumentName, // Unlike SoundfontProvider.ts this is not optional.
...
export class SoundfontProvider extends Component<
  ...
  ProviderState
> {
  // We use a default value this way in a class.
  ...
  public componentDidUpdate({
    instrument: prevInstrument, // Getting previous state, destructured, renamed.
  }: ProviderProps) {
    const { instrument } = this.props;
  ...
  private load = async (instrument: InstrumentName) => {
    ...
    // A second setState is OK because we have an await, so, time will pass.
